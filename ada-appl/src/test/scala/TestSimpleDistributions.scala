...
class TestExp3Reward extends Properties("TestExp3Reward"){
    ...
    property("Exp3Reward update works for 0 < rewards < 1 ") = Prop.forAllNoShrink{ (v: Double, gamma: Double, k: Int, vals: List[Double]) =>
        val gammaVal = math.abs(gamma%10)
        val kVal = math.max(1, math.abs(k))
        val vals2 = vals.map(v => math.abs(v%1.0))
        val distribution = new Exp3Reward(math.abs(v%1.0), gammaVal, kVal )
        if (vals.length > 2 && vals.length < 5){
            val rewards = vals2.map(vv => {distribution.update(new Reward(vv)); distribution.draw})
            val tests = rewards.drop(1).zip(rewards.zip(vals2)).map{case(r2, (r1, vv)) => if(gammaVal* vv/kVal >= 0.0) r2 >= r1 else r1 < r2}
            tests.reduce(_&&_)
        } else{
            true
        }
