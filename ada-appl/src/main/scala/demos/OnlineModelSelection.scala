...
class OnlineModelSelection{
    ...
    val models = (101 until 106).map{i => 
        new OnnxClassifier[Int, Array[Array[Array[Array[Float]]]], Int, BetaDistribution](f"/home/leon/data/onnx/ada-example/conv-seed${i}.onnx", "input", i => i)
    }
    ...
    def trainEnsemble(nIterL: List[Int] = List(1000)): List[List[Double]] = {
        val ensemble = ThompsonSamplingEnsemble[Int, Array[Array[Array[Array[Float]]]], Int]((0 until 5).zip(models).toMap, (0 until 5).map{i => (i, new BetaDistribution(1, 1, 1.0))}.toMap)
        ...
        val selectionsList = nIterL.map{nIter => 
            (0 until nIter).map{i =>
                ...
                val (action, modelIds) = ensemble.actWithID(data(pick), Stub())
                val reward =  new Reward( if(Utilities.extractSingleValue(action) == labels(pick)) 1.0 else 0.0 )
    ...
    def measureRecall(): Unit = {
        ...
        val correct = predictions.zipWithIndex.map{ case(preds, i) => {
            preds.map(pred => if(Utilities.extractSingleValue(pred) == labels(i)) 1.0 else 0.0)
        }}
    ...
    def applyModels(data_path: String, labels_path: String, fine_labels_path: String, threshold: Double = 0.001, silent: Boolean = true): (ListBuffer[List[Tree[Int]]], ListBuffer[Int],  ListBuffer[Int]) = {
        val rnd = util.Random
        val lines = Source.fromFile(data_path).getLines
        val labelslines = Source.fromFile(labels_path).getLines
        val finelabelslines = Source.fromFile(fine_labels_path).getLines
        ...
        val predictionss = ListBuffer[List[Tree[Int]]]()
        val labels = ListBuffer[Int]()
    ...
    def print_predictions(label: Int, predictions: List[Tree[Int]]): Unit = {
        print(f"target: ${label}")
        print("\tpredictions: ")
        predictions.map(pred => print(f" ${Utilities.extractSingleValue(pred)}"))
        println()
    }
}